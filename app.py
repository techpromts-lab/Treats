...
def load_css(dark=False):
    ...
    <style>
        ...
        .stApp {{ -webkit-overflow-scrolling: touch; overscroll-behavior-y: contain; }}
        ...
        * {{ -webkit-tap-highlight-color: transparent; }}
        ...
        .sidebar-label {{
            ...
            letter-spacing: 0.08em; color: {text_muted}; padding: 8px 14px 6px 14px; margin-top: 8px;
        }}
        ...
        label[data-baseweb="radio"]:has(input:checked) {{ background: {active} !important; border-color: {border} !important; font-weight: 700 !important; }}
        ...
        .user-card img {{ width: 38px; height: 38px; border-radius: 50%; border: 2px solid #ddd6fe; flex-shrink: 0; }}
        ...
        .login-sub {{ font-size: 15px; color: {text_soft}; margin: 0 0 36px 0; }}
        .login-footer {{
        ...
        .token-bar-wrap {{ height: 6px; background: {border}; border-radius: 3px; overflow: hidden; margin: 8px 0 4px 0; }}
        .token-bar-fill {{ height: 100%; border-radius: 3px; transition: width 0.3s ease; }}

        section.main, [data-testid="stMain"] {{
            display: flex !important; flex-direction: column !important;
            align-items: center !important; width: 100% !important;
        }}
        section.main > div.block-container, [data-testid="stMain"] > div.block-container, .main .block-container {{
            width: 100% !important; max-width: 780px !important;
            margin-left: auto !important; margin-right: auto !important;
            padding: 1.5rem 1.5rem 5rem 1.5rem !important;
        }}

        [data-testid="stChatInput"], [data-testid="stChatInputContainer"], [data-testid="stBottomBlockContainer"] {{
        ...
        [data-testid="stChatInput"] {{
            border-radius: 14px !important; border: 1px solid {btn_border} !important;
            background: {chat_input_bg} !important; box-shadow: 0 4px 16px rgba(108, 62, 245, 0.06) !important;
        }}
        [data-testid="stChatInput"]:focus-within {{ border-color: #a855f7 !important; box-shadow: 0 4px 20px rgba(168, 85, 247, 0.15) !important; }}
        [data-testid="stChatInput"] textarea {{ background: transparent !important; color: {text} !important; }}
        [data-testid="stChatInput"] textarea::placeholder {{ color: {text_soft} !important; }}

        h1, h2, h3, h4, h5, h6 {{ color: {text} !important; }}
        h1 {{ font-size: 30px !important; font-weight: 800 !important; letter-spacing: -0.03em !important; }}
        ...
        .stButton > button:hover {{ background: {btn_hover}; border-color: #a855f7; color: #6c3ef5; transform: translateY(-1px); }}
        ...
        [data-testid="stForm"] {{
            ...
            padding: 22px 24px; background: {surface};
        }}
        ...
        details {{
            ...
            padding: 4px 14px; background: {surface_2}; color: {text} !important;
        }}
...
You are friendly, concise, and always ready to help. Never use emojis unless the user uses them first."""

TITLE_PROMPT = """Generate a short title (3-5 words) for this conversation.
...
def render_login_page():
    ...
    st.markdown(
        '<div class="login-footer">Treats v3.0 · Powered by Groq</div>',
        unsafe_allow_html=True,
    )
...
def init_conversations():
    if "conversations" not in st.session_state:
        st.session_state.conversations = {
            "default": {"id": "default", "title": "New chat", "messages": [], "created": datetime.now().isoformat()}
        }
...
def new_conversation():
    ...
    st.session_state.conversations[new_id] = {
        "id": new_id, "title": "New chat", "messages": [], "created": datetime.now().isoformat()
    }
...
with st.sidebar:
    ...
    if is_unlimited():
        ...
        if pct >= 90:
            bar_color, status_class, status_text = "#ef4444", "danger", "Critical"
        elif pct >= 70:
            bar_color, status_class, status_text = "#f59e0b", "warn", "Warning"
        else:
            bar_color, status_class, status_text = "#10b981", "dev", "Active"
    ...
    choice_label = st.radio("Navigation", list(TOOLS.keys()), label_visibility="collapsed", key="nav")
    ...
    for i, msg in enumerate(messages):
        with st.chat_message(msg["role"]):
            ...
            if msg["role"] == "assistant":
                ...
                st.markdown(f'<div class="token-badge">{tokens} tokens</div>', unsafe_allow_html=True)
                if i == len(messages) - 1:
...
CV_TEMPLATES = {
    ...
    "Classic": "Use a traditional professional layout with clear sections.",
    "Creative": "Use a creative layout with unique section styling.",
}
...
def render_cv():
    ...
    if not can_send():
        st.warning("Daily token limit reached.")
        return
...
def render_tts():
    ...
    if not avail:
        ...
        return
    lang_choice = st.radio("Language", avail, format_func=lambda x: LANG_LABELS.get(x, x), horizontal=True)
...
def render_photo():
    ...
    prompt = st.text_area("Prompt", placeholder="A cat sipping coffee in a Parisian cafe, cinematic lighting, 4K", height=110)
    col1, col2, col3 = st.columns(3)
    with col1:
    ...
    dims = {"1:1 (1024×1024)": (1024, 1024), "16:9 (1344×768)": (1344, 768),
    ...
    if st.button("Generate Image", type="primary", use_container_width=True):
        ...
        try:
            with st.spinner("Generating image..."):
                img_bytes = generate_image(prompt=prompt, width=w, height=h,
    ...
    if "last_image" in st.session_state:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.download_button("Download PNG", data=st.session_state["last_image"],
            ...
            if st.button("Regenerate", key="photo_regen", use_container_width=True):
                ...
                try:
                    with st.spinner("Regenerating..."):
                        img_bytes = generate_image(prompt=st.session_state["last_prompt"],
                            width=w, height=h, model=model_img, seed=new_seed, enhance=enhance)
                    st.session_state["last_image"] = img_bytes
                    st.rerun()
                except TreatsError as e:
                    st.error(str(e))
